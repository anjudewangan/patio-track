...
const StatusCard = ({
  ...
  const positionItems = useAttributePreference(
    "positionItems",
    "speed,address,totalDistance,course,status,deviceTime,voltage,power,lastUpdate,batteryLevel,signalStrength"
  );
  ...
  const handleShare = useCatchCallback(async () => {
    ...
    const response = await fetch("/api/devices/share", {
      ...
      body: new URLSearchParams(
        `deviceId=${deviceId}&expiration=${expiration}`
      ),
    });
    if (response.ok) {
    ...
    <>
      ...
      <Dialog open={Boolean(shared)} onClose={() => setShared(null)}>
        ...
        <DialogActions>
          ...
          <Button onClick={() => navigator.clipboard?.writeText(shared)}>
            {t("sharedCopy")}
          </Button>
        </DialogActions>
      </Dialog>
