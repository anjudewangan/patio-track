...
const DeviceStatusReportPage = () => {
  ...
  useEffect(async () => {
    if (queryParams) {
      const response = await fetch(`/api/reports/onlineReport?${queryParams}`);
      if (response.ok) {
        setChartData(await response.json());
      }
    } else {
      const response = await fetch(`/api/reports/onlineReport`);
      if (response.ok) {
        setChartData(await response.json());
      }
    }
...
export default DeviceStatusReportPage;
